...
describe Spree::LineItem do
  let(:line_item) {create(:wholesale_line_item, wholesale_price: 0.0)}
  ...
  let(:order) { create(:non_wholesale_order_with_line_items, line_items_count: 2, line_items_price: 19.99) }
  ...
  context "wholesale line_items" do
    ...
    it "should get wholesale price" do
      ...
      expect(wholesale_line_item.wholesale_price).to eq(wholesale_line_item.variant.wholesale_price)
    end
  ...
  context '#copy_price' do
    it "copies over a variant's prices" do
      ...
      expect(wholesale_line_item.price).to eq(variant.price)
      expect(wholesale_line_item.cost_price).to eq(variant.cost_price)
      expect(wholesale_line_item.currency).to eq(variant.currency)

      expect(wholesale_line_item.wholesale_price).to eq(variant.wholesale_price)
    end
  end
  ...
  context '#update_price' do
    it 'copies over a variants differing price for another vat zone' do
      ...
      expect(wholesale_line_item.price).to eq(wholesale_line_item.variant.price)
      expect(wholesale_line_item.wholesale_price).to eq(wholesale_line_item.variant.wholesale_price)
    end
    ...
    it 'copies over a variants differing price for another vat zone for non-wholesaleable order' do
      ...
      expect(item.wholesale_price).to eq(12.00)
    end
  end
